...
const SIMPLE_SCENES = {
	SCENE_KEY.EXPLOSION: preload("res://shooter/explosion/explosion.tscn"),
	SCENE_KEY.BOOM: preload("res://shooter/explosion/explosion.tscn")
}

const POWER_UP_SCENE: PackedScene = preload("res://shooter/power_up/power_up.tscn")

func get_random_powerup():
	return GameData.POWER_UPS.keys().pick_random()

func add_child_deferred(child_to_add, parent: Node2D) -> void:
	if is_instance_valid(parent) == true:
...
func create_boom(start_pos: Vector2) -> void:
	create_simple_scene(start_pos, SCENE_KEY.EXPLOSION, get_tree().current_scene)
